  ...
  return (
    <div style={s.wrap}>
      ...
      <div style={s.status}>
        ...
        <span style={{fontFamily:FONT,fontSize:10,fontWeight:500,color:'#4F46E5'}}>
          {t('scanning', locale)}
...
const s = {
  wrap:  { flex:1,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',gap:13,padding:'26px 22px',background:'#F8FAFC',animation:'fadeSlideUp .4s ease',position:'relative',overflow:'hidden' },
  glow:  { position:'absolute',top:'30%',left:'50%',transform:'translate(-50%,-50%)',width:240,height:240,borderRadius:'50%',background:'radial-gradient(circle,rgba(79,70,229,0.04) 0%,transparent 70%)',pointerEvents:'none' },
  tag:   { fontFamily:"'DM Sans',sans-serif",fontSize:9,fontWeight:600,color:'#4F46E5',letterSpacing:'.1em',textTransform:'uppercase' },
  shell: { width:120,height:120,borderRadius:14,display:'flex',alignItems:'center',justifyContent:'center',position:'relative',overflow:'hidden',background:'linear-gradient(145deg,rgba(79,70,229,0.06),rgba(11,31,58,0.04))',border:'1px solid rgba(79,70,229,0.15)',boxShadow:'inset 0 2px 0 rgba(255,255,255,0.6),inset 0 -2px 0 rgba(15,23,42,0.04),0 10px 28px rgba(15,23,42,0.08)' },
  shellShine: { position:'absolute',top:0,left:0,right:0,height:'42%',background:'linear-gradient(180deg,rgba(255,255,255,0.5),transparent)',borderRadius:'14px 14px 0 0',pointerEvents:'none' },
  fpSvg: { width:76,height:88,position:'relative',zIndex:1 },
  scanBar:  { position:'absolute',left:'-5%',right:'-5%',height:2,background:'linear-gradient(90deg,transparent,rgba(79,70,229,0.2),rgba(79,70,229,1),rgba(79,70,229,0.2),transparent)',animation:'fpscan 2.5s ease-in-out infinite',boxShadow:'0 0 10px rgba(79,70,229,0.6),0 0 3px rgba(79,70,229,0.3)' },
  scanGlow: { position:'absolute',left:'-5%',right:'-5%',height:32,background:'linear-gradient(180deg,transparent,rgba(79,70,229,0.05),transparent)',animation:'fpscan 2.5s ease-in-out infinite',marginTop:-15 },
  status:   { display:'flex',alignItems:'center',gap:6 },
  statusDot:{ width:4,height:4,borderRadius:'50%',background:'#4F46E5',animation:'blink 1s infinite' },
  hint:  { fontFamily:"'DM Sans',sans-serif",fontSize:9,color:'#94A3B8',textAlign:'center' },
