...
export default function BiometricScan() {
  ...
  return (
    <div style={s.wrap}>
      ...
      {/* Accurate fingerprint SVG in clay+liquid scanner shell */}
      <div style={s.shell}>
      ...
      <div style={s.status}>
        ...
        <span style={{fontFamily:FONT,fontSize:10,fontWeight:500,color:'#9d7dfd'}}>
          {t('scanning', locale)}
          <span style={{display:'inline'}}><span style={s.ell}>.</span><span style={{...s.ell,animationDelay:'.15s'}}>.</span><span style={{...s.ell,animationDelay:'.3s'}}>.</span></span>
        </span>
      </div>
...
const s = {
  wrap:  { flex:1,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',gap:13,padding:'26px 22px',background:'linear-gradient(160deg,#04040a,#0a0a16)',animation:'fadeSlideUp .4s ease',position:'relative',overflow:'hidden' },
  glow:  { position:'absolute',top:'30%',left:'50%',transform:'translate(-50%,-50%)',width:240,height:240,borderRadius:'50%',background:'radial-gradient(circle,rgba(124,92,252,0.07) 0%,transparent 70%)',pointerEvents:'none' },
  tag:   { fontFamily:"'DM Sans',sans-serif",fontSize:9,fontWeight:600,color:'rgba(124,92,252,0.65)',letterSpacing:'.1em',textTransform:'uppercase' },
  shell: { width:120,height:120,borderRadius:14,display:'flex',alignItems:'center',justifyContent:'center',position:'relative',overflow:'hidden',background:'linear-gradient(145deg,rgba(91,63,212,0.13),rgba(20,20,36,0.28))',border:'1px solid rgba(160,140,255,0.1)',boxShadow:'inset 0 2px 0 rgba(255,255,255,0.06),inset 0 -2px 0 rgba(0,0,0,0.4),0 10px 28px rgba(0,0,0,.7)' },
  shellShine: { position:'absolute',top:0,left:0,right:0,height:'42%',background:'linear-gradient(180deg,rgba(255,255,255,0.055),transparent)',borderRadius:'14px 14px 0 0',pointerEvents:'none' },
  fpSvg: { width:76,height:88,position:'relative',zIndex:1 },
  scanBar:  { position:'absolute',left:'-5%',right:'-5%',height:2,background:'linear-gradient(90deg,transparent,rgba(157,125,253,0.2),rgba(157,125,253,1),rgba(157,125,253,0.2),transparent)',animation:'fpscan 2.5s ease-in-out infinite',boxShadow:'0 0 10px rgba(157,125,253,0.85),0 0 3px #fff' },
  scanGlow: { position:'absolute',left:'-5%',right:'-5%',height:32,background:'linear-gradient(180deg,transparent,rgba(124,92,252,0.07),transparent)',animation:'fpscan 2.5s ease-in-out infinite',marginTop:-15 },
  status:   { display:'flex',alignItems:'center',gap:6 },
  statusDot:{ width:4,height:4,borderRadius:'50%',background:'#7c5cfc',animation:'blink 1s infinite' },
  hint:  { fontFamily:"'DM Sans',sans-serif",fontSize:9,color:'#3e3e58',textAlign:'center' },
