...
export default function BiometricScan() {
  ...
  return (
    <div style={s.wrap}>
      ...
      <div style={s.status}>
        <div style={s.sdot}/>
        <span style={{fontFamily:FONT,fontSize:14,fontWeight:500,color:'#9d7dfd'}}>
          {t('scanning',locale)}
          <span>
...
const s = {
  ...
  tag:{fontFamily:"'DM Sans',sans-serif",fontSize:11,fontWeight:600,color:'rgba(124,92,252,0.65)',letterSpacing:'.1em',textTransform:'uppercase'},
  shell:{
    width:'clamp(160px,20vw,220px)',height:'clamp(160px,20vw,220px)',
    borderRadius:20,display:'flex',alignItems:'center',justifyContent:'center',
    position:'relative',overflow:'hidden',
    background:'linear-gradient(145deg,rgba(91,63,212,0.13),rgba(20,20,36,0.28))',
    border:'1px solid rgba(160,140,255,0.1)',
    boxShadow:'inset 0 2px 0 rgba(255,255,255,0.06),inset 0 -2px 0 rgba(0,0,0,0.4),0 16px 40px rgba(0,0,0,.7)',
  },
  shellShine:{
  ...
  scanBar:{
    ...
    background:'linear-gradient(90deg,transparent,rgba(157,125,253,0.2),rgba(157,125,253,1),rgba(157,125,253,0.2),transparent)',
    animation:'fpscan 2.5s ease-in-out infinite',
    boxShadow:'0 0 12px rgba(157,125,253,0.9),0 0 4px #fff',
  },
  scanGlow:{
    position:'absolute',left:'-5%',right:'-5%',height:40,
    background:'linear-gradient(180deg,transparent,rgba(124,92,252,0.07),transparent)',
    animation:'fpscan 2.5s ease-in-out infinite',marginTop:-19,
  },
  status:{display:'flex',alignItems:'center',gap:8},
  sdot:{width:6,height:6,borderRadius:'50%',background:'#7c5cfc',animation:'blink 1s infinite'},
  hint:{fontFamily:"'DM Sans',sans-serif",fontSize:13,color:'#3e3e58',textAlign:'center'},
